...
<?php
// Determine extra body classes based on page/template
if (is_page_template('page-login.php')) {
    $extra_classes = '';
} elseif (is_page_template('page-register.php')) {
    $extra_classes = 'bg-[#EDEDED]'; // Example: registration background
} elseif (is_page_template('page-stats.php')) {
    $extra_classes = 'bg-[#1E2330]';
} elseif (is_page_template('edit-profile.php')) {
    $extra_classes = 'bg-[#F9F9F9]'; // Example: edit profile background
} elseif (get_query_var('current_user_obj')) {
    $extra_classes = 'bg-[#F3F3F1]'; // User profile pages
} else {
...
>
  ...
  <div :class="isSticky ? 'fixed top-0 left-0 right-0 z-50' : 'z-50'"
     class="transition-all duration-300 lg:px-16 lg:pt-4">
    <div class="p-4">
      <div class="flex items-center justify-between rounded-full px-4 bg-white py-3 lg:py-4 lg:mb-0 lg:mb-0 pl-8">
        ...
        <div class="flex items-center">
          ...
          <nav class="hidden lg:flex space-x-10 ml-16 text-sm">
            <a href="/adventures" class="hover:text-[#124C12] transition">How it work's</a>
            <a href="/overview" class="hover:text-[#124C12] transition">Get started</a>
          </nav>
        </div>
            ...
            <div class="flex items-center space-x-4 lg:space-x-6">
               <!-- Desktop Find Foragers -->
                ...
                foreach ($users as $user):
                    ...
                    $avatar = $profile_image ?: get_avatar_url($user_id);
                    $display_name = esc_html($user->display_name);
                    $slug = strtolower(str_replace(' ', '-', $user->user_login));
            ...
            <?php if (is_user_logged_in()): ?>
              ...
              <a
                ...
                class="hidden lg:flex items-center gap-2 bg-[#CEE027] text-[#111827] px-5 py-4 rounded-xl font-medium text-sm"
              >
            ...
            <?php else: ?>
              ...
              <a
                href="mk/login"
                class=" lg:flex items-center gap-2 bg-[#CEE027] text-[#111827] px-5 py-4 rounded-full font-medium text-sm"
              >
                <span>Log in</span>
              </a>
            <?php endif; ?>
            ...
            <?php if (is_user_logged_in()):
                  ...
                  $display_name = esc_html($user->display_name);
                  $user_slug   = strtolower(str_replace(' ', '-', $user->user_login));
              ...
              <div x-data="{ open: false }" class="relative w-10 h-10">
                  ...
                  <button @click="open = !open" class="relative w-10 h-10 focus:outline-none">
                      ...
                      if (!empty($profile_image)) {
                          $avatar = esc_url($profile_image); // ✅ custom uploaded avatar
                      } else {
                          $avatar = get_avatar_url($user_id); // fallback → Gravatar / default WP avatar
                      }
            ...
            <div x-data="{ open: false }" class="lg:hidden relative">
                        ...
                        <div class="absolute left-8 top-[44px] text-sm font-regular">Meny</div>
                          ...
                          <div class="mt-auto space-y-4">
                            <?php if (is_user_logged_in()): ?>
                              ...
                              <a href="<?php echo wp_logout_url(home_url('/')); ?>" class="block text-center bg-gray-200 py-4 rounded-xl">Logout</a>
                            <?php else: ?>
